...
Agents must be able to upload an image file and deliver it to FB Messenger contacts via the same `send-image` endpoint used for LINE.

#### Scenario: Agent sends image on FB channel
...
- **THEN** the route returns HTTP 501 `NOT_IMPLEMENTED`
