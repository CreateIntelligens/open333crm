...
Define the workflow for importing external LINE Flex Message JSON into tenant-scoped Materials. Users design Flex structures in LINE Flex Simulator, paste the finished JSON into CRM, validate it with LINE's Messaging API, preview it with a LINE-compatible renderer, and save it as a reusable `line_flex_template` Material.
