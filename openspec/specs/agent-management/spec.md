...
The API SHALL provide `PATCH /api/v1/agents/:id/password` to allow an `ADMIN` to reset any agent's password without requiring the current password.

#### Scenario: Admin resets agent password
- **GIVEN** a request is authenticated with role `ADMIN`
- **WHEN** `PATCH /api/v1/agents/:id/password` is called with `{ newPassword }`
- **THEN** the response SHALL be HTTP 200 with body `{ message: "Password reset" }`

#### Scenario: Non-admin is rejected
- **GIVEN** a request is authenticated with role `SUPERVISOR` or `AGENT`
- **WHEN** `PATCH /api/v1/agents/:id/password` is called
- **THEN** the response SHALL be HTTP 403

#### Scenario: Target agent not found
- **GIVEN** a request is authenticated with role `ADMIN`
- **WHEN** `PATCH /api/v1/agents/:id/password` is called with a non-existent agent ID
- **THEN** the response SHALL be HTTP 404

---

### Requirement: Deactivate Agent
The API SHALL provide `DELETE /api/v1/agents/:id` to allow an `ADMIN` to deactivate an agent account (set `isActive = false`). Deactivated agents SHALL NOT be returned in `GET /api/v1/agents` and SHALL NOT be able to log in.

#### Scenario: Admin deactivates agent
- **GIVEN** a request is authenticated with role `ADMIN`
- **WHEN** `DELETE /api/v1/agents/:id` is called
- **THEN** the response SHALL be HTTP 204
- **AND** the agent SHALL no longer appear in `GET /agents`
...
#### Scenario: Target agent not found
- **GIVEN** a request is authenticated with role `ADMIN`
- **WHEN** `DELETE /api/v1/agents/:id` is called with a non-existent agent ID
- **THEN** the response SHALL be HTTP 404
