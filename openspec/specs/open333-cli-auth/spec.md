## Purpose
Provide a publishable Open333 CLI with API-backed CLI sessions, scoped token authentication, local credential storage, server status checks, and curated CLI API discovery without changing browser or partner API authentication behavior.
