...
export function TemplatePicker({ open, onClose, onSelect, channelType }: TemplatePickerProps) {
  ...
  if (selected) {
    ...
    return (
      <div className="absolute bottom-0 left-0 right-0 z-30 flex max-h-[60%] flex-col rounded-t-lg border-t border-surface-line bg-background shadow-lg">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-surface-line px-4 py-2">
          <div className="flex items-center gap-2">
            <button onClick={handleBack} className="text-ink-subtle hover:text-foreground">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-ink-subtle">填入變數值：</p>
            {keys.map((key) => (
              <div key={key} className="flex items-center gap-2">
                <label className="min-w-[120px] text-sm text-ink-subtle">
                  {getVarLabel(key)}
                </label>
                <Input
                  value={varValues[key] || ''}
          ...
          <div className="rounded-md border bg-neutral-20 p-3">
            <p className="mb-1 text-xs font-medium text-ink-subtle">預覽</p>
            <p className="whitespace-pre-wrap text-sm">{previewText}</p>
          </div>
        </div>

        {/* Footer */}
        <div className="border-t border-surface-line px-4 py-2 flex justify-end">
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30 flex max-h-[50%] flex-col rounded-t-lg border-t border-surface-line bg-background shadow-lg">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-surface-line px-4 py-2">
      ...
      <div className="px-4 py-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-md border border-input bg-background pl-8 pr-3 py-1.5 text-sm placeholder:text-ink-subtle focus:outline-none focus:ring-2 focus:ring-ring"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {isLoading ? (
          ...
          <p className="py-8 text-center text-sm text-ink-subtle">沒有找到模板</p>
        ) : (
          templates.map((tpl: TemplateItem) => {
            ...
            return (
              ...
              >
                ...
                {tpl.description && (
                  <p className="mt-0.5 text-xs text-ink-subtle">{tpl.description}</p>
                )}
                <p className="mt-1 truncate text-xs text-ink-subtle/70">{bodyText}</p>
              </button>
            );
          })
        )}
