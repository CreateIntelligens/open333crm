    ...
    <div className="flex h-screen flex-col bg-muted">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="mx-auto max-w-6xl space-y-5">
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-white">
            ...
            {!isLoading && materials.length === 0 && (
              <div className="py-16 text-center">
                <div className="text-sm text-muted-foreground">尚未建立任何素材</div>
          ...
          {meta && (
            <div className="text-xs text-muted-foreground">共 {meta.total} 個素材</div>
          )}
        </div>
      </main>
    </div>
  ...
  return (
    <tr className="border-b border-border last:border-b-0 hover:bg-muted">
      <td className="px-4 py-3">
        ...
        {material.description && <div className="mt-0.5 text-xs text-muted-foreground">{material.description}</div>}
      </td>
      <td className="px-4 py-3 text-xs text-muted-foreground">
        {material.template?.name ?? '—'}
        <div className="text-[10px] text-muted-foreground font-mono">{material.contentType}</div>
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-xs">
        {material.variables.length === 0 ? (
          ...
          <span className="text-foreground">{material.variables.length} 個</span>
        )}
      </td>
      <td className="px-4 py-3 text-right tabular-nums text-xs">{material.usageCount}</td>
      <td className="px-4 py-3 text-xs text-muted-foreground tabular-nums">{new Date(material.updatedAt).toISOString().slice(0, 10)}</td>
      <td className="relative px-4 py-3 text-right">
        ...
        {menuOpen && (
          <div className="absolute right-4 top-full z-10 mt-1 w-32 rounded-md border border-border bg-white py-1 shadow-md">
            <button
              type="button"
