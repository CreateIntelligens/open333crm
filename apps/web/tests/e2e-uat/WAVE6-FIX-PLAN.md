...
## 尚未處理（需使用者決定或需部署）
- **端到端驗證 P0**：需部署 UAT 才能驗 `/s/{slug}` 回 302、`/fan/auth` 回 200
- **CM-171/CM-172 未併回 main**：`6b4305f`、`cec1ff3` 只在
  `feat/platform-user-management`，從 main 重新部署會整包回歸
- 工單分類三處不一致 → 要統一成哪一套詞彙表（需產品決定）
- LINE 影片 endCard 永遠不送出 → 補實作 vs 移除 UI（需產品決定）
- 工單列表頁無建立入口 → 是否要加
- 送出訊息 content 零驗證 → 需先確認各 contentType 的合法 payload 結構
- SLA / 門戶積分 Int4 溢位 500
- `/knowledge/search` 冷啟動連續 504
- system prompt 4 欄位無長度上限
- DB 清理（120 條軟刪規則、12 筆 ENDED 活動）
- **push / 開 PR / 部署 —— 一律需明確同意，尚未執行**
