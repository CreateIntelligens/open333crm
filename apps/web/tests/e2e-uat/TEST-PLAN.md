...
## 零、撰寫規範（2026-09-23 新增，違反過才補的）

Wave 6 寫了 338 個欄位案例，仍讓三種 bug 上線。以下是從那次回推出來的硬規則。

### 1. 正負案例必須成對，正向優先

**每個欄位都要有「合法值存得進去、讀得回來、值不變」的往返案例。**

反例（短連結 `expiresAt`，Wave 6 當時的寫法）：

```ts
test('expiresAt：非法日期字串應被擋（不應 500）', async () => {
  // 只送 'not-a-date'，只斷言 >= 400
});
```

只測「壞輸入被擋」。後來驗證被加嚴成 `z.string().datetime({ offset: true })`，
前端 `<input type="datetime-local">` 送的 `2026-12-31T23:59` 也一起被擋死，
**短連結完全建不出來，而這個測試照樣全綠**——它從來沒問過合法輸入能不能通過。

當時 15 個建立短連結的測試，沒有一個帶過合法的 `expiresAt`。

> 漏掉負向案例只是少擋一個邊界；漏掉正向案例是功能全掛。加嚴驗證時，
> 先補正向案例再改 schema。

### 2. 用前端真正會送的值，不是自己想當然的格式

API 直測很方便，但送的值要跟瀏覽器一致。日期欄位要送：

| 來源 | 實際送出 |
|---|---|
| `<input type="datetime-local">` | `2026-12-31T23:59`（無時區、無秒） |
| `<input type="date">` | `2026-12-31` |
| `new Date().toISOString()` | `2026-12-31T23:59:00.000Z` |

Wave 6 全部只送第三種，所以測不出前兩種被擋掉。不確定前端送什麼，
就去看那個 `<Input>` 再寫。

時區也一樣：前後端若都不處理時區，往返測試會「一致」但語意是錯的
（短連結到期時間曾差 8 小時），這種 bug 要對照絕對時間才驗得出來。

### 3. `test.fail()` 是暫時標記，不是結案方式

標記時**在註解寫下「為什麼這次不修」**，而不是只寫 bug 現象。

官網活動的空標題／純空白／超長標題／結束早於開始，Wave 6 當時就測出來了，
註解也寫了「後端無 zod」，但標成 `test.fail` 之後套件全綠，就這樣放著。
後來同一輪錯誤訊息中文化改過那個檔案 64 行，卻沒補這幾個驗證
——標記變成了不修的藉口。

- 修好後 Playwright 報 unexpected pass，要把標記翻回正常 `test`，並實跑確認會過
- **交付前掃一次**：`grep -c "test.fail" *.spec.ts`
- 2026-09-23 盤點：36 個（cases-contacts-inbox 12、channels-portal 15、marketing-line 9）
...
- 8 個案例用 `test.fail()` 標記「預期失敗」記錄真 bug —— 修好後會轉成 unexpected pass 自動提醒移除標記
  （⚠️ 但標記本身不等於修好，見「零、撰寫規範」第 3 點——這批標記後來確實被放著沒處理）
...
unexpected pass 主動提醒移除標記。

⚠️ **但它只是暫時標記，不是結案**。這批標記在 2026-09-23 之前一直沒人處理，
其中官網活動那 4 個（空標題／純空白／超長標題／結束早於開始）在後續的
錯誤訊息中文化那輪還改過同一個檔案，卻沒順手補上驗證。規範見「零、撰寫規範」第 3 點。
