...
function uaSimulatorEnabled(): boolean {
  return process.env.NODE_ENV !== 'production' || process.env.SHORTLINK_UA_SIMULATOR === '1';
}
...
export default async function shortlinkRedirectRoutes(app: FastifyInstance) {
  ...
  app.post('/track', async (request, reply) => {
    ...
    if (!slug) {
      return reply.status(400).send({ success: false, error: { code: 'BAD_REQUEST', message: 'slug required' } });
    }
    ...
    if (!targetUrl) {
      return reply.status(410).send({ success: false, error: { code: 'GONE', message: 'Link not found or expired' } });
    }
  ...
  app.get('/:slug', async (request, reply) => {
    ...
    const { cid, ua_simulator } = request.query as { cid?: string; ua_simulator?: string };

    const resolved = await getLinkForRedirect(app.prisma, slug);
    const override = uaSimulatorEnabled() ? parseSourceOverride(ua_simulator) : null;
    ...
    if (!resolved) {
      ...
      return reply.status(expired.status).headers(expired.headers ?? {}).send(expired.html);
    }
    ...
    return reply.status(result.status).headers(result.headers ?? {}).send(result.html);
  });
}
