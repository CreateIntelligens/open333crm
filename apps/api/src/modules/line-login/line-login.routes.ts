...
export default async function lineLoginRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{
    Querystring: { lineUid: string; channelId: string };
  }>('/authorize', async (request, reply) => {
    const { lineUid, channelId } = z
      .object({
    ...
    const url = generateAuthUrl(lineUid, channelId);
    return reply.send(success({ url }));
  });

  // ─── GET /callback ── Public: LINE Login OAuth callback ──────────────────
  fastify.get<{
    Querystring: { code?: string; state?: string; error?: string };
  }>('/callback', async (request, reply) => {
    const { code, state, error } = request.query;

    // Frontend result page base URL
    const frontendBase = process.env.NEXT_PUBLIC_WEB_URL || 'http://localhost:3000';
    const resultPage = `${frontendBase}/line-login/result`;

    // User cancelled or LINE returned an error
    if (error || !code || !state) {
    ...
    try {
      ...
      const { email, userId } = await verifyIdToken(idToken);

      if (!email) {
        fastify.log.warn({ userId }, 'LINE Login: no email in ID token');
      ...
      fastify.log.info(
        ...
        'LINE Login: contact email updated',
      );

      return reply.redirect(`${resultPage}?status=success`);
    } catch (err) {
      fastify.log.error({ err }, 'LINE Login callback error');
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      // 2. Find the LINE channel identity for this contact + channel
      const identity = conversation.contact?.channelIdentities?.find(
        (ci) => ci.channelId === conversation.channelId,
      );

      if (!identity) {
        return reply.status(400).send({ success: false, error: { code: 'BAD_REQUEST', message: '找不到聯絡人的 LINE 身份' } });
      }

      // 3. Generate the authorization URL
      const authUrl = generateAuthUrl(identity.uid, conversation.channelId);

      // 4. Send the link via LINE Bot (push message)
      const channel = conversation.channel;
      if (!channel?.isActive) {
        return reply.status(400).send({ success: false, error: { code: 'BAD_REQUEST', message: 'LINE 渠道未啟用' } });
      }
      ...
      // Build a friendly message with the auth link
      ...
      if (!sendResult.success) {
        fastify.log.error({ error: sendResult.error }, 'Failed to send LINE email request message');
        return reply.status(502).send({ success: false, error: { code: 'CHANNEL_ERROR', message: '發送 LINE 訊息失敗' } });
      }
