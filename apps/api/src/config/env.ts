...
const envSchema = z.object({
  ...
  LICENSE_PROVIDER: z.enum(['allow-all', 'env', 'cache']).default('allow-all'),
  LICENSE_FAIL_CLOSED: z.string().transform((v) => v === 'true').default('false'),
  LICENSE_CACHE_TTL_MS: z.coerce.number().int().positive().default(300000),
  LICENSE_CACHE_SOURCE: z.enum(['allow-all', 'env']).default('allow-all'),
  LICENSE_FEATURES_JSON: z.string().optional(),
  LICENSE_LIMITS_JSON: z.string().optional(),
  LICENSE_CREDITS_JSON: z.string().optional(),
  LICENSE_CHANNELS_JSON: z.string().optional(),
