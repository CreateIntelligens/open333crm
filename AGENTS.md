...
- **Errors**: `throw new AppError(message, code, statusCode, details?)` (`apps/api/src/shared/utils/response.ts`) — **message comes first**. `code` defaults to `INTERNAL_ERROR` and `statusCode` to 500, so pass both. No raw error returns
