...
export class AutomationEngine {
  static async start() {
    ...
    EventBus.subscribe(async (event: BaseEvent) => {
      try {
        ...
        logger.error(
          `[AutomationEngine] Failed handling event ${event.type}:`,
          err,
        );
      }
    });
  ...
  private static async handleEvent(event: BaseEvent) {
    ...
    for (const rule of rules) {
      try {
        ...
        if (
          !engineRule.conditions ||
          Object.keys(engineRule.conditions).length === 0
        ) {
        ...
        if (events.length > 0) {
          logger.info(
            `[AutomationEngine] Rule ${rule.id} matched event ${event.type}. Executing actions...`,
          );

          const actions = events[0].params as Array<{
            type: string;
            params: any;
          }>;
          let success = true;
        ...
        logger.error(
          `[AutomationEngine] Error evaluating rule ${rule.id}:`,
          rErr,
        );
      }
    }
  ...
  private static async executeAction(
    ...
    logger.debug(
      `[AutomationEngine] Executing action ${action.type}`,
      action.params,
    );
    ...
    switch (action.type) {
      ...
      case "create_case":
        if (!event.payload.contactId)
          throw new Error("Missing contactId in event for create_case action");
      ...
      case "send_message":
        ...
        logger.info(
          `[AutomationEngine] STUB: send_message to ${action.params.channelId}: ${action.params.text}`,
        );
        break;
