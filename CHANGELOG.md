...
### Added
- **License/Billing strategy mode (2026-06-11)** — 新增 `add-license-billing-strategy` 規劃與實作：目前沒有 license server，預設 `LICENSE_PROVIDER=allow-all`，所有未知功能、限制、點數、channel 皆先允許；可切換 `LICENSE_PROVIDER=env` 以 `LICENSE_FEATURES_JSON`、`LICENSE_LIMITS_JSON`、`LICENSE_CREDITS_JSON`、`LICENSE_CHANNELS_JSON` 控制方案內容；可切換 `LICENSE_PROVIDER=cache` 先讀 cache/Redis snapshot，miss 時依 `LICENSE_CACHE_SOURCE` fallback，未來接 license API 只需新增 provider strategy。API 存取權呼叫方式統一走 `LicenseService`：route preHandler 用 `requireFeature('feature.key')` / `requireCredits('creditType', amount)`；需要數量限制時呼叫 `licenseService.checkLimit(key, current, { tenantId })`；channel 建立呼叫 `licenseService.checkChannelCreation(channelType, currentCount, { tenantId })`；訊息費用呼叫 `getMessageFee()` + `checkCredits()` / `deductCredits()`。功能本身只定義 feature key 並在 route/service 邊界詢問 LicenseService，不直接判斷月付方案名稱。
